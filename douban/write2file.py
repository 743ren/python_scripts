...
def write_2_md(path: Path, book: Book, tag: str = ''):
    ...
    with open(mdPath, 'w', encoding='UTF-8') as f:
        ...
        if book.sub_title:
            f.write(f'副标题: "{book.sub_title}"\n')
        if book.origin_title:
            f.write(f'原作名: "{book.origin_title}"\n')
        if book.author:
        ...
        if book.publish_year:
            f.write(f'出版年: {book.publish_year}\n')
...
def get_language(book: Book):
    ...
    if re.search(r'\[美\]|\[英\]', book.author):
        ...
        language = '韩语'
    elif book.origin_title:
        try:
            # 防止书名都是数字这种无法找到语言特征的
            lan = detect(book.origin_title)
            language = {
              'en': '英语',
              'fr': '法语',
              'de': '德语',
